...
final class DonationService {
    ...
    private func mapDonation(doc d: QueryDocumentSnapshot) -> Donation? {
        let x = d.data()
    ...
    func fetchForDonor(donorId: String, completion: @escaping ([Donation]) -> Void) {
        db.collection("donations")
            .whereField("donorId", isEqualTo: donorId)
            .getDocuments { [weak self] snap, _ in
                guard let self = self else { return }
                let docs = snap?.documents ?? []
                completion(docs.compactMap { self.mapDonation(doc: $0) })
            }
    }

    func fetchForNgo(ngoId: String, completion: @escaping ([Donation]) -> Void) {
        db.collection("donations")
